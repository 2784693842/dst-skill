...
| 原子 skill | 在换皮流水线中的职责 |
|---|---|
| `modtool-automation` | 用 `.dmt` 工程导入原 zip → 解出 Build/Symbol/帧 → 导出每帧 PNG → 处理后回填 → 另存 `.dmt` 工程（**不是 zip**）。**唯一能程序化操作 DST Mod Tool 的入口** |
...
原 anim zip (D:\...\boat_ancient.zip)
    ...
    ② 遍历 build.symbols → 逐符号用 frame:export_png() 导出原帧 PNG 到工作目录
    │
    ▼ [sensenova-vision]
    ...
    │
    ▼ [modtool-automation]
    ⑨ 用 frame:replace_image() 把新纹理 PNG 按符号路径回填到 .dmt 工程
       （实测 replace_image 保留原 width/height，尺寸无需手动匹配）
    ⑩ tool:save_document_as(<新>.dmt) 另存为 .dmt 工程（不覆盖原 zip）
    │
    ▼ [dst-assets-animation-atlas]
    ⑩' 把另存的 .dmt 交给 atlas 编译流水线 → 产出 atlas-*.tex + build.bin → 打包成新 zip
    │
...
> **⚠️ 关键结论（实测验证）**：`tool:save_document_as` **只能写 `.dmt`（或无扩展名路径），不能直接产出 `.zip`**（实测报错 `save_document_as path must have no extension or a .dmt extension`）。DST 动画包（zip 内为 `atlas-*.tex` + `build.bin`）是编译产物，必须由 `dst-assets-animation-atlas` 的 SCML/KTEX 编译打包生成。所以：**⑩ 存工程，⑩' 编译打包**，两步缺一不可。
...
- **原 zip 永不覆盖**：所有产物写入独立工作目录，**另存为 `.dmt` 工程**（非 zip），再经 atlas 编译打包成新 zip
- **像素尺寸与 pivot 保留原值**：实测 `frame:replace_image()` 保留原 width/height（`before 196x35 → after 196x35`），**无需手动匹配尺寸**；导出时用 `frame:export_png()` 保证与编译产物一致
...
用户提到"dmt 辅助"时，指的就是用 DST Mod Tool 的 `.dmt` 工程文件作为程序化操作载体（`modtool-automation` skill 的 `tool:open_document` / `tool:save_document_as`）。本 skill 全程围绕 `.dmt` 工程展开：导入原 zip → 在工程内替换纹理 → 另存 `.dmt` → 交给 atlas 编译成 zip。

**版本前置检查（必做）**：`tool:open_document` / `tool:save_document_as` / `import_resources` 是 Mod Tool **1.1.13** 才有的能力。流水线开始前必须先确认：
1. 运行 `DST Mod Tool.exe --help`，确认输出 `dst-app [FILES...]` / `dst-app script (...)` 用法（老版本会直接弹 GUI）
2. 运行 `DST Mod Tool.exe script --help`，**grep `save_document_as` 与 `open_document`**（实测这两个 API 在 1.1.13 的 script 帮助里存在；**`--help` 顶层没有版本号，版本确认靠 script 帮助内容**）
...
- 原 zip 与新打包 zip 的 Build/Symbol/帧数完全一致（只换纹理，不改结构）
- 新纹理经 sensenova-vision 识读，画风判定与原帧一致
- **新纹理识读确认右下角无水印文本**（watermark=false 是否真的生效）
- 导出关键帧（idle/run/attack 若有）识图校验外观符合预期
- 新 zip 能被 DST 正常加载（用 atlas skill 的 KTEX 校验 tex/bin 打包完整性）
- **原 zip 文件不落盘修改**：回填产物全部写入新 `.dmt`，新可发布 zip 由 atlas 编译生成；**不做**"原 zip 字节 sha256 对比"（Mod Tool 回填必然重写字节，且最终 zip 是编译产物，该断言不可执行）
