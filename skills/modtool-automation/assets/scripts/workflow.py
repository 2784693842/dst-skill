...
def open_file(exe: str | None = None, file_path: str | None = None) -> subprocess.CompletedProcess:
    ...
    注意: 文件参数通过 --open 传递。工具启动后 GUI 窗口打开，
    后续操作通过 lua_run 注入脚本。
    """
...
def run_lua(
    ...
    示例:
      >>> result = run_lua("print(document_revision)")
...
def select_animation(name: str, exe: str | None = None) -> LuaResult:
    """选择动画"""
    old = os.environ.get("DST_ANIMATION_NAME")
    ...
    try:
        ...
        if old is None:
            ...
            os.environ["DST_ANIMATION_NAME"] = old


def select_frame(idx: int, exe: str | None = None) -> LuaResult:
    """选择帧（0-based）"""
    old = os.environ.get("DST_FRAME_INDEX")
    ...
    try:
        ...
        if old is None:
            ...
            os.environ["DST_FRAME_INDEX"] = old
