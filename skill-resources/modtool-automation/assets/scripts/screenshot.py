...
def capture(
    ...
    # 使用 user32.ClientToScreen 精确转换客户区坐标为屏幕坐标
    region_left, region_top, region_w, region_h = region
    client_pt = wintypes.POINT()
    client_pt.x = region_left
    client_pt.y = region_top
    user32.ClientToScreen(hwnd, ctypes.byref(client_pt))
    screen_left = client_pt.x
    screen_top = client_pt.y
...
def client_to_screen(
    ...
    注意:
        使用 user32.ClientToScreen() 精确计算，自动处理标题栏/边框偏移。
    """
    pt = wintypes.POINT()
    pt.x = client_x
    pt.y = client_y
