...
def semantic_diff(before: Image, after: Image, workdir: str,
    ...
    """
    语义级对比：将两张图拼接为上下布局后传给识图 API。

    调用 caption-vision.ps1（sensenova-vision skill 的核心脚本）。
    先保存两张临时 PNG，再拼接为"上图=操作前，下图=操作后"的对比图，
    传给识图 API 做差异描述。
    ...
    # 拼接为上下对比图（上图=操作前，下图=操作后），确保两张图尺寸一致
    if before.size != after.size:
        after = after.resize(before.size)
    w, h = before.size
    combined = Image.new("RGB", (w, h * 2))
    combined.paste(before, (0, 0))
    combined.paste(after, (0, h))
    combined_path = os.path.join(workdir, "combined.png")
    ...
    return f"[识图不可用] 截图已保存: {before_path}, {after_path}, {combined_path}"
